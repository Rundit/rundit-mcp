...
export const TOOLS: ToolSpec[] = [
  ...
  {
    name: "metrics_import",
    description: "Preview or import metrics across many companies in one call\n\nImport a fund in one request: companies contains up to 100 companies, each with its native currency and up to 50 metric items; maximum 1000 items and 10000 points overall. Resolve types by id or exact name, optionally create custom definitions, request missing rows through the VC template, and write actual/forecast/budget points. No instance-id discovery needed. Defaults to dryRun=true; rejects conflicting stored values unless overwrite is explicit. Access and currency are validated for all companies before writes. Results follow input order and return counts and ids, never point echoes. Check success and item statuses; apply is not atomic. Existing targeting is preserved. Dynamic scopes: metric-types:write for type creation, metric-templates:write for template changes. Replays reuse compatible types and skip identical points. Template changes can notify companies; forecast/budget flags affect existing targets and require full targeting access. Dates are period starts. No formulas or forecast intervals.",
    annotations: {"readOnlyHint":false,"destructiveHint":true},
    inputSchema: {
      "type": "object",
      "properties": {
        "companies": {
          "minItems": 1,
          "maxItems": 100,
          "description": "Companies to import, each with its native source currency and metric items. Each company may appear once. Maximum 1000 metric items and 10000 points across the request. Access and currency are checked for every company before any writes.",
          "type": "array",
          "items": {
            "type": "object",
            "properties": {
              "companyId": {
                "type": "number",
                "description": "One accessible portfolio company. New template targeting adds this company only; existing targeting is never replaced."
              },
              "currency": {
                "type": "string",
                "description": "Source currency, e.g. USD. Must equal company native currency. No implicit FX conversion on writes."
              },
              "items": {
                "minItems": 1,
                "maxItems": 50,
                ...
                "items": {
                  ...
                  "properties": {
                    "metricTypeId": {
                      "type": "number",
                      "description": "Use exactly one of metricTypeId, metricTypeName or createType."
                    },
                    "metricTypeName": {
                      "type": "string",
                      "description": "Exact case-insensitive name or shortName. Unknown or ambiguous names fail; no fuzzy writes."
                    },
                    "createType": {
                      "description": "Create only if no accessible type matches the name. Retries reuse a VC-owned full-name match with identical configuration; mismatches return a conflict.",
                      "allOf": [
                        {
                          "$ref": "#/components/schemas/SdkCreateMetricTypeDto"
                        }
                      ]
                    },
                    "flavor": {
        ...
        "dryRun": {
          "type": "boolean",
          "default": true,
          "description": "Defaults to true: validates and previews without creating types, templates, rows or points. Set false to apply. Each item reports its own status; the batch is not atomic."
        },
        "onConflict": {
          "type": "string",
          "enum": [
          ...
          "default": "reject",
          "description": "Default rejects an item if any stored point differs. Identical values are skipped on retries. overwrite explicitly replaces conflicting stored values."
        }
      },
      "required": [
        "companies"
    ...
    invoke: (client, args) => client.metrics.import(args),
  },
  {
    name: "metrics_search",
    ...
    inputSchema: {
      ...
      "properties": {
        ...
        "includePoints": {
          "type": "boolean",
          "default": true,
          "description": "Set false to enumerate metric ids, types, units and forecast/budget rows without loading or computing point histories. Returns empty points arrays. Date/timeframe/pointLimit filters only apply when points are included."
        },
        "companyIds": {
          "description": "Restrict results to these companies. Defaults to all companies the caller can access.",
          "type": "array",
